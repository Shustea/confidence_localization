...
def save_cached_sample(out_path: Path, rtf: torch.Tensor, labels: torch.Tensor, title: str, meta=None, overwrite: bool = False):
...
def load_cached_sample(path: Path):
    ...
    return rtf.float(), labels, str(title)
...
def save_prediction_plots(
    ...
    plot_dir = output_dir if output_dir.endswith("/") else output_dir + "/"
    safe_stem = re.sub(r"[^A-Za-z0-9._-]+", "_", filename_stem)
    save_sample_as_image(doa.cpu(), labels.cpu(), bound.cpu(), f"{safe_stem}.png", title=title, path=plot_dir)
